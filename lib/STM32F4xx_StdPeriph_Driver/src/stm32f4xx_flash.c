...
void flash_write(uint32_t data,uint8_t data_address)
{
...
//	read_data = *(__IO uint32_t*)startAddress+0x0ffc;
	 
  FLASH_EraseSector(FLASH_Sector_11, VoltageRange_3);
   
	FLASH_ProgramWord((startAddress+data_address), data);
	
  /* Lock the Flash to disable the flash control register access (recommended
     to protect the FLASH memory against possible unwanted operation) *********/
  FLASH_Lock(); 
}
uint32_t flash_read(uint8_t data_address)
{
	read_data = *(__IO uint32_t*)(startAddress+data_address);	
	return read_data;
}
